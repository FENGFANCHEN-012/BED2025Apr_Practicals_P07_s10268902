...
app.put("/books/:id",async(req,res)=>{
  ...
  if (result.rowsAffected[0] === 0) {
          ...
          return res.status(500).json("internal error")
           ...
           res.send("book "+id+" has been changed")
        }

  }

  catch(error){
    console.error("Error occur during PUT process"+error)
...
app.delete("/books/:id", async (req,res)=>{
  ...
  try{
   ...
   connection = await sql.connect(dbConfig)
    ...
    const result = request.query(`DELETE FROM books WHERE id = @id`)
    if (result.rowsAffected[0] === 0) {
            return res.status(404).json({ error: `Book with ID ${student_id} not found` });
        }
      ...
      return res.status(204).json("No content")
          ...
          return res.status(500).json("internal error")
           ...
           res.send("book "+bookid+" has been delete from database")
        }
